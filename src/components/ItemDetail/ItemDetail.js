...
const ItemDetail = ({
  ...
  if (!itemId && !item.barcode) {
    return <FormattedMessage id="ui-requests-mediated.actions.loading" />;
  }
  ...
  const positionLink = count
    ...
    : count;
  const itemLabel = item.barcode ? 'ui-requests-mediated.item.barcode' : 'ui-requests-mediated.item.id';
  ...
  return (
    <>
      <Row>
        ...
        <Col xs={4}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.title" />}>
            {title}
          </KeyValue>
        </Col>
        <Col xs={4}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.contributorNames" />}>
            {contributor}
          </KeyValue>
        </Col>
      </Row>
      <Row>
        <Col xs={4}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.effectiveLocation" />}>
            {effectiveLocationName}
          </KeyValue>
        </Col>
        <Col xs={8}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.callNumber" />}>
      ...
      <Row>
        <Col xs={4}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.status" />}>
            {statusMessage}
          </KeyValue>
        </Col>
        <Col xs={4}>
          <KeyValue label={<FormattedMessage id="ui-requests-mediated.item.dueDate" />}>
            {dueDate}
          </KeyValue>
        </Col>
        <Col
        ...
        >
          <KeyValue
            label={<FormattedMessage id="ui-requests-mediated.item.requestsOnItem" />}
            value={positionLink}
          />
        </Col>
      </Row>
