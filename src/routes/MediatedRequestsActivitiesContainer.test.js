...
describe('buildQuery', () => {
  ...
  it('should include search fields when building CQL query', () => {
    expect(buildQuery(queryParams, pathComponents, resources, logger)).toEqual(expect.stringContaining(
      'instance.title=="*queryValue*" or item.barcode=="*queryValue*" or requester.barcode=="*queryValue*" or searchIndex.callNumberComponents.callNumber=="*queryValue*") sortby sortValue'
    ));
  });
});
